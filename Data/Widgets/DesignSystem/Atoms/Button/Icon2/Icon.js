...
async function loadTextAsset(fileName) {
	...
	if (!response.ok) {
		throw new Error(`Failed to load ${fileName} for Atom.button.icon@2.0.0.`);
	}
...
const TEMPLATE = await loadTextAsset('icon.html');
ensureCssLoaded('icon.css');
...
function render(target, options = {}, resolveDependency) {
	...
	const iconEl = button.querySelector('.ds-btn__icon');
	if (iconEl) {
