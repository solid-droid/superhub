...
export async function handlePluginFullBundle({ params, services, projectRoot }) {
  ...
  try {
    ...
    return jsonResponse(200, { ok: true, data: bundle });
  } catch (error) {
    return jsonResponse(500, toErrorBody("plugin resolution failed", error.message));
  }
}

export async function handlePluginFullFile({ params, services, projectRoot }) {
  const slug = getPathParam(params, "slug");
  const version = getPathParam(params, "version");
